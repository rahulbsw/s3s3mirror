...
public class S3ToS3Options implements AWSCredentials {
    ...
    public static final String USAGE_CTIME = "Only copy objects whose Last-Modified date is younger/older than this many days. " +
            "For other time units, use these suffixes: y (years), M (months), d (days), w (weeks), h (hours), m (minutes), s (seconds). "+
            "For use suffix - (younger), + (older)";
    ...
    private long initMaxAge() {
        ...
        String ctime=this.ctime.replaceAll("(\\+|-)","");
    ...
    private int getCtimeNumber(String ctime) {
        if (ctime.matches("^[0-9]+$")) return Integer.parseInt(ctime);
        else if (ctime.matches("^[0-9]+[yMwdhms]$")) return Integer.parseInt(ctime.substring(0,ctime.length()-1));
        else return Integer.parseInt(ctime.substring(0,ctime.length()-2));
    }

    @Getter private long nowTime = System.currentTimeMillis();
    @Getter private long age;
    @Getter private String ageDate;
    @Getter private boolean isYounger;
    ...
    public void initDerivedFields() {
        ...
        if (hasCtime()) {
            this.age = initMaxAge();
            this.ageDate = new Date(age).toString();
            this.isYounger=!(ctime.indexOf('+')>0);
        }
